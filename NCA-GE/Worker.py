...
class Worker():
	...
	def calculate_kendall_tau(self):
		...
		k_first = int(self.graph.get_num_nodes() * Constants.K_FIRST)
		id_top_x = np.flip(self.graph.norm_cent.argsort()[-k_first:], axis=0)
		for idx in id_top_x:
			x.append(rank_x[idx])
			y.append(rank_y[idx])
		#for a, b in zip(x, y):
			#print "[", a, ", ", b, "]"
			#print "[", a, ", ", b, "] ===> (", self.graph.norm_cent[a], ", ", self.graph.predictions[a], ")"
